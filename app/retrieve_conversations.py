        ...
        with self.driver.session() as session:
            if matching_message_id:
                ...
                """,
                    ...
                    match_id=matching_message_id,
                    before=context_window // 2,
                    after=context_window // 2,
                )
            else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Retrieve conversations using GraphRAG"
    )
    ...
    parser.add_argument(
        "--purpose", help="Filter by purpose (debugging, learning, etc.)"
    )
    parser.add_argument("--tone", help="Filter by tone (humorous, serious, neutral)")
    parser.add_argument(
        "--complexity", help="Filter by complexity (beginner, intermediate, advanced)"
    )
    parser.add_argument(
        "--domain", help="Filter by domain (programming, web_dev, etc.)"
    )
    parser.add_argument(
        "--no-context", action="store_true", help="Don't show conversation context"
    )
    parser.add_argument(
    ...
    try:
        if args.interactive or not args.query:
            ...
            while True:
                ...
                purpose = input(
                    "  Filter by purpose (or press Enter to skip): "
                ).strip()
                if purpose:
            ...
            if args.json:
                ...
                results = retriever.vector_search(
                    args.query, limit=args.limit, tags=tags if tags else None
                )
